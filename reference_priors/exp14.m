...
k=100;          % number of samples in each set
...
sigma_err_min=0.01;     % min values of sigma_err
sigma_err_max=10;       % max values of sigma_err
t=0:0.5:10;             % time at which cumulative infiltraion is available

% list of parameters at which prior is to be computed
mu_ks_list=0.01:1:10;          % list of values of mu_ks
sigma_ks_list=0.01:1:10;    % list of  values of sigma_ks
sigma_err_list=0.01:1:10;   % list of values of sigma_err
...
for mu_ks_ind=1:length(mu_ks_list)                      % loop of mu_ks values
    for sigma_ks_ind=1:length(sigma_ks_list)            % loop for sigma_ks values
        for sigma_err_ind=1:length(sigma_err_list)      % loop for sigma_err values
            ...
            sigma_err_tmp=sigma_ks_list(sigma_err_ind);
            ...
            for i=1:n                                   % loop for each set of samples
                ...
                par_samps=[unifrnd(mu_ks_min,mu_ks_max,100,1),unifrnd(sigma_ks_min,sigma_ks_max,100,1)];
