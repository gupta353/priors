...
fname = 'GP_train_test_data_9.mat';
...
%
ind = find(Xtrain(:,3)<=log_ks_thresh & Xtrain(:,2)<=0.2 & Xtrain(:,2)>0.12 & Xtrain(:,4)>2);
Xtrain = Xtrain(ind,:);
ytrain = ytrain(ind,:);

ind = find(Xtest(:,3)<=log_ks_thresh & Xtest(:,2)<=0.2 & Xtest(:,2)>0.12 & Xtest(:,4)>2);
Xtest = Xtest(ind,:);
ytest = ytest(ind,:);
%}
...
% theta = [3.83725590398492,58.2038832567709,0.845848297377934,2.25091552624403,38.0059216943300,18.9781881066915,10.5694430397704,1.59803044458439];
 theta = [0.0590541034107446,41.6039996553150,0.324614278052968,105.303469537237,60.9550843854456,0.260922904673809,30.2361636112521,7.30553304783942];
sigf2 = theta(6);
l = 1./theta(1:5);
M = diag(l);
