...
fname = 'GP_train_test_data_8.mat';
...
%
ind = find(Xtrain(:,3)<=log_ks_thresh & Xtrain(:,2)<=1 & Xtrain(:,2)>0.2 & Xtrain(:,4)<=0.2);
Xtrain = Xtrain(ind,:);
ytrain = ytrain(ind,:);

ind = find(Xtest(:,3)<=log_ks_thresh & Xtest(:,2)<=1 & Xtest(:,2)>0.2 & Xtest(:,4)<=0.2);
Xtest = Xtest(ind,:);
ytest = ytest(ind,:);
%}
...
% theta = [3.83725590398492,58.2038832567709,0.845848297377934,2.25091552624403,38.0059216943300,18.9781881066915,10.5694430397704,1.59803044458439];
 theta = [1.00056984591882,8.46842375466187,0.129884494892087,5.24868920161807,12.6013056885643,14.8801630218265,14.4566476878719,1.96853876478305];
sigf2 = theta(6);
l = 1./theta(1:5);
M = diag(l);
