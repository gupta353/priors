...
fname = 'GP_train_test_data_7.mat';
...
%
ind = find(Xtrain(:,3)<=log_ks_thresh & Xtrain(:,2)<=1 & Xtrain(:,2)>0.2 & Xtrain(:,4)<=2 & Xtrain(:,4)>0.2);
Xtrain = Xtrain(ind,:);
ytrain = ytrain(ind,:);

ind = find(Xtest(:,3)<=log_ks_thresh & Xtest(:,2)<=1 & Xtest(:,2)>0.2 & Xtest(:,4)<=2 & Xtest(:,4)>0.2);
Xtest = Xtest(ind,:);
ytest = ytest(ind,:);
%}
...
% theta = [3.83725590398492,58.2038832567709,0.845848297377934,2.25091552624403,38.0059216943300,18.9781881066915,10.5694430397704,1.59803044458439];
 theta = [1.12788764045792,25.2063411933342,0.554130340696586,57.7433953790646,37.2654396593715,0.481471730610351,8.96871048766180,9.77277762390096];
sigf2 = theta(6);
l = 1./theta(1:5);
M = diag(l);
