...
T2=zeros(n_ks,length(t)-1);           % 
mu_omega=zeros(1,length(t)-1);
    ...
    for K_ind=1:length(K_samps)     % loop for each value of K in K_samps
        ...
        if K_tmp<Kc_tmp
            ...
            F(K_ind,t_ind)=Fp(K_ind,t_ind-1)+...
                sqrt(2*K_tmp*psi_delta_theta)*(t_tmp^0.5-tp(K_ind,t_ind-1)^0.5)+...
...
I=r.*(1-mu_omega)+mean(T2);     % Infiltration rate during each time-interval
end