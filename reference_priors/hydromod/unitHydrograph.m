...
mean_RTD_stream=(length_num_mat(1:s_order_max,4))/vel_stream;           % column 4 of 'length_num_mat' contains average length of each stream order
mean_RTD_overland=ddensity^(-1)/(2*vel_hillslope);
%% computation of unit hydrograph
t=support;

% efficient computation of unit hydrograph
%
% compute exponential term of the convolution and store in the matrix

lambdas=[1/mean_RTD_overland,1./mean_RTD_stream'];
exp_term=nan(length(t),length(lambdas));
 for lambda_ind=1:length(lambdas)
     exp_term(:,lambda_ind)=exp(-lambdas(lambda_ind)*t);
 end
...
for trans=1:length(avail_trans)
    ...
    temp_uh_fun=expconv(lambda);

    f_gamma=exp_term(:,1)*temp_uh_fun(1);
    for j=2:length(temp_uh_fun)         % j=2 corresponds to first reach in the path and second lambda value
          f_gamma=f_gamma+exp_term(:,temp_trans(j-1)+1)*temp_uh_fun(j);
    end
    uh=uh+p(trans)*f_gamma;
end
%}

% reader-friendly computation of unit hydrograph
%{
uh=zeros(length(t),1);
for trans=1:length(avail_trans)
    temp_trans=avail_trans{trans};
    lambda=[1/mean_RTD_overland,1./mean_RTD_stream(temp_trans)'];
    temp_uh_fun=expconv(lambda);
    f_gamma=exp(-lambda(1)*t)'*temp_uh_fun(1);
    for j=2:length(temp_uh_fun)
        f_gamma=f_gamma+exp(-lambda(j)*t)'*temp_uh_fun(j);
    end
    uh=uh+p(trans)*f_gamma;
end
%}
